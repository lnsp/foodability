    ...
    <div class="flex-grow">
      <div class="flex items-center">
        ...
        <div class="flex items-center flex-wrap gap-x-4 gap-y-2 ml-4">
          ...
          <input type="number"
                 v-model="days"
                 class="border-2 bg-gray-100 rounded-lg border-green-600 px-4 py-2 text-lg w-16 text-center">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center flex-wrap gap-x-4 gap-y-1">
          ...
          <form @submit.prevent="add()">
            <input type="text"
                   class="px-4 py-2 bg-gray-100 border-2 border-green-600 focus:border-green-900 w-full rounded-lg"
                   placeholder="banana"
                   v-model="current">
      ...
      <transition-group name="fade"
        ...
        <div v-for="tag in tags"
             :key="tag"
             class="bg-green-700 hover:bg-green-800 text-sm px-2 py-1 text-white rounded flex items-center w-min">
          <span>{{ tag }}</span>
          <button class="p-1 cursor-pointer"
                  @click.prevent="drop(tag)">
...
export default {
  ...
  methods: {
    ...
    next() {
      ...
      this.$store.commit('food/tags', this.tags)
      this.$store.commit('food/days', this.days)
